  ...
  useEffect(() => {
    ...
    const handleFetchCourses = async () => {
      try {
        ...
        const res = await axios.get(`${import.meta.env.VITE_BACKEND_API}/course/all`);
        console.log(res);
              ...
              <div className="p-4 flex flex-col grow">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className="flex items-center text-yellow-500 text-sm font-bold">
                    <span>★</span>
                    <span className="ml-1 text-gray-700">{c.rating || "0.0"}</span>
                    <span className="text-gray-400 text-xs font-normal ml-1">({c.noOfRating || 0})</span>
                  </div>
                </div>
                ...
                <p className="text-sm text-gray-600 mb-4">
                  by <span className="font-medium text-gray-800">{c.author}</span>
                </p>

                {/* Footer / Price / Action */}
                <div className="mt-auto pt-4 border-t border-gray-100 flex justify-between items-center">
                  <span className="text-lg font-bold text-green-600">Free</span>
                  <button onClick={()=> navigate(`/courses/${c._id}`)} className="px-4 py-2 bg-gray-900 text-white text-sm font-medium rounded-lg hover:bg-gray-800 transition-colors cursor-pointer">
                    View Details
                  </button>
                </div>
              </div>
